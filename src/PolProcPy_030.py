#!/usr/bin/python
"""GUI v3.0 for the Polonator Processor machine."""
...
#===============================================================================
#       Software Version
        self.aboutVersion.setText("Version 0.3.0")
        self.aboutDate.setText("08/06/2010")
#===============================================================================
...
#===============================================================================
    def proc_start(self, cmd):
        ...
        global proc
        proc = QProcess()
        proc.start(cmd, OpenMode="ReadOnly")
    ...
    def proc_readyRead(self):
        ...
        outWin.append(str(proc.readLine()).strip('\n'))
        log.write(str(proc.readLine()).strip('\n'))
        log.close()
...
#===============================================================================
    ...
    def polonatorinitialize(self):
        ...
        self.polonator_process.setEnabled(False)
        global conds
        conds = ['self.polonator_initialize.setEnabled(False)',
                 'self.polonator_process.setEnabled(False)']
        self.updateProcesses1()
    ...
    def polonatorprocess(self):
        ...
        global args
        global outWin
        nextFunc = "self.polonatorprocess2()"
        args = ['self.polonator_initialize.setEnabled(False)',
                'self.polonator_process.setEnabled(False)']
        outWin = self.polonator_textarea
        self.proc_start(cmd)

    def polonatorprocess2(self):
        """Continuation of polonatorprocess2()."""
        global conds
        conds = ['self.polonator_initialize.setEnabled(False)',
                 'self.polonator_process.setEnabled(False)']
        self.updateProcesses1()
    ...
    def polonatorkill(self):
        ...
        cmd = 'perl process_kill.pl'
        global nextFunc
        global args
        global outWin
        nextFunc = "self.polonatorkill2()"
        args = ['pass']
        outWin = self.polonator_textarea
        self.proc_start(cmd)

    def polonatorkill2(self):
        """Continuation of polonatorkill()."""
        global conds
        conds = ['self.tab_1.setEnabled(True)', 'self.tab_2.setEnabled(True)',
...
#===============================================================================
    ...
    def start_QC(self):
        ...
        for item in pendlist:
            ...
            cmd1 = ("perl disp_delta.pl tetrahedra/" + seltext +
                    "-QC.delta QC-" + seltext + "-delta")
            cmd2 = "perl disp_regQC.pl " + seltext
            cmdlist1.append(cmd1)
            cmdlist2.append(cmd2)
        self.start_QC_proc_st()
        return

    def start_QC_proc_st(self):
        """Coninuation of start_QC()."""
        global nextFunc
        nextFunc = "self.start_QC_proc2()"
        self.proc_start(cmdlist1[nn])
...
#===============================================================================
    ...
    def histogram_DrawHistogram(self):
        ...
        if self.histogramCycleButton.isChecked() == True:
            subcmd = "perl histogram4.pl"
        else:
            subcmd = "perl histogram.pl"
    ...
    def histogram_GenerateData(self):
        ...
        if a!=-1:
            ...
            cmd = "perl makePrimerFile.pl " + str(selectedFile)
            global nextFunc
            global args
            nextFunc = "self.proc_pass()"
    ...
    def basecaller_RunBasecallerButton(self):
        ...
        try:
            ...
            while i < length:
                ...
                outfile.write(value)
                outfile.write("\n")
                self.polnator_textarea.append('wrote "' + str(value) +
                                                   '" to "' + cmd1 + '"' )
                i = i+1
            outfile.close()
        except IOError:
            self.polnator_textarea.append('"' + cmd1 + '" failed ')
            
        cmd2 = ("perl basecall.pl " + str(self.basecallerReadfileFN.text()) +
...
#===============================================================================
    ...
    def updateProcesses1(self):
        ...
        """
        cmd1 = 'perl initialize_processor.pl'
    ...
    def updateProcesses2(self):
        ...
        cmd2 = "perl find_running_processes.pl"
        path = os.path.realpath("")
        self.polonator_experiment.setText(path)
        self.polonator_textarea.append(path)
        global nextFunc
        global args
        global outWin
        nextFunc = "self.updateProcesses3()"
    ...
    def updateProcesses3(self):
        """Continuation of updateProcesses2."""
        print "updateProcesses3"
        cmd3 = "/home/polonator/PROCESS_STATUSS"
        self.polonator_process.setEnabled(False)
        self.polonator_initialize.setEnabled(False)
        try:
            print "updateProcesses3 Try"
            r3 = open(cmd3)
            out3 = r3.readlines()
            worked = True
            r3.close()
        except IOError:
            print "updateProcesses3 Except"
            self.polonator_textare.append('"' + cmd3 + '" failed to open')
            worked = False
        if worked == True:
            print "updateProcesses3 Worked"
            print str(out3)
            for line in out3:
                line = line.strip("\n")
    ...
    def updateQualityLists(self):
        """
        Update the lists in the Complerte Cycles,
